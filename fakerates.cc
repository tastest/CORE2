...
bool isFakeableMuon (int index, enum fakeRateVersion version){
	cout << endl  << "fakerates.cc is depricated. Fakeable object definitions are now in muonSelections.cc" <<  endl << endl;
	gSystem->Exit(1);
	return false;
} 
...
bool isFakeableElectron (int index, enum fakeRateVersion version){
	cout << endl  << "fakerates.cc is depricated. Fakeable object definitions are now in electronSelections.cc" <<  endl << endl;
	gSystem->Exit(1);
	return false;
}
